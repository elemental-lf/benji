...
class Transform(TransformBase):
    ...
    def __init__(self, *, config, name, module_configuration):
        ...
        master_key = Config.get_from_dict(module_configuration, 'masterKey', None, types=bytes)
        if master_key is not None:
            ...
            kdf_salt = Config.get_from_dict(module_configuration, 'kdfSalt', types=bytes)
            kdf_iterations = Config.get_from_dict(module_configuration, 'kdfIterations', types=int)
            password = Config.get_from_dict(module_configuration, 'password', types=str)
