...
class Tree:
    ...
    def dir(self, size=0, date=None):
        return dict(st_mode=(S_IFDIR | 0o700),
                    st_nlink=0,
                    st_size=size,
    ...
    def file(self, size=0, date=None):
        return dict(st_mode=(S_IFREG | 0o600),
                    st_nlink=1,
                    st_size=size,
                    st_ctime=self._time(date),
                    st_mtime=self._time(date),
    ...
    def blk(self, size=0, date=None):
        ...
        return dict(st_mode=(S_IFBLK | 0o660),
                    st_nlink=2,
                    st_size=0,
                    st_ctime=self._time(date),
                    st_mtime=self._time(date),
    ...
    def symlink(self, size=0, date=None):
        return dict(st_mode=(S_IFLNK | 0o777), st_nlink=1, st_size=size)
...
class BenjiFuse(LoggingMixIn, Operations):
    ...
    def _tree(self):
        ...
        for version in self.backy.meta_backend.get_versions():
            ...
            tree.create(_name_path,
                        tree.file(size=len(version.name), date=version.date),
                        data=version.name.encode('utf-8'))
            _expire_path = os.path.join(version_uid_path, 'expire')
            _expire_data = version.expire.isoformat() if version.expire else ''
            tree.create(_expire_path,
                        tree.file(size=len(_expire_data), date=version.date),
                        data=_expire_data.encode('utf-8'))
            _snapshot_name_path = os.path.join(version_uid_path, 'snapshot_name')
            tree.create(_snapshot_name_path,
                        tree.file(size=len(version.snapshot_name), date=version.date),
                        data=version.snapshot_name.encode('utf-8'))
            if version.valid:
