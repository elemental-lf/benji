...
function benji::versions_status {
    local INCOMPLETE_VERSIONS=$(benji -m --log-level "$BENJI_LOG_LEVEL" ls 'status == "incomplete" and date < "1 day ago"' | jq '.versions | length')
    local OLDER_INVALID_VERSIONS=$(benji -m --log-level "$BENJI_LOG_LEVEL" ls 'status == "invalid"' | jq '.versions | length')

    benji_older_incomplete_versions set $OLDER_INVALID_VERSIONS
    benji_invalid_versions set $INVALID_BACKUPS
}
